...
def main(incident_id):
    ...
    print(f"POST {url} with REJECT")
    resp = requests.post(url, headers=headers, json={"decision": "REJECT", "reason": "Testing reject path"})
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_reject.py <incident_id>")
        sys.exit(1)
    main(sys.argv[1])
